...
RSGCore.Functions.CreateCallback('newspaper:server:getStories', function(source, cb)
    local src = source
...
RegisterNetEvent('newspaper:buy', function(type)
    ...
    if type then
        if cash >= Config.Price then
            ...
            TriggerClientEvent('RSGCore:Notify', source, '$' .. Config.Price .. ' required for buying a newspaper',
                'error')
        end
    end
end)
...
RegisterNetEvent('newspaper:server:updateStory', function(data)
    ...
    if Player.PlayerData.job['name'] == 'reporter' then
        if not knownPlayers[source] then
            ...
            TriggerClientEvent('RSGCore:Notify', src, 'Story has been updated!', 'success')
        end
    else
        TriggerClientEvent('RSGCore:Notify', src, 'You need to be a reporter to update a story', 'success')
    end
...
RegisterNetEvent('newspaper:server:publishStory', function(data)
    ...
    if Player.PlayerData.job['name'] == 'reporter' then
        if not knownPlayers[source] then
            ...
            TriggerClientEvent('RSGCore:Notify', src, 'Story has been published!', 'success')
        end
    else
        TriggerClientEvent('RSGCore:Notify', src, 'You need to be a reporter to publish a story', 'success')
    end
...
RegisterNetEvent('newspaper:server:deleteStory', function(data)
    ...
    if Player.PlayerData.job['name'] == 'reporter' then
        if not knownPlayers[source] then
            ...
            TriggerClientEvent('RSGCore:Notify', src, 'Story have been deleted', 'success')
        end
    else
        TriggerClientEvent('RSGCore:Notify', src, 'Not possible to delete story', 'success')
    end
